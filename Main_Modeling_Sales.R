...
Main_Data.pois<-Main_Data %>%
  ...
  filter(complete.cases(Sales, huest, EQ_L36_6))


Var_list<-Main_Data.pois %>% 
  dplyr::select(starts_with("EQ"), HOUST.log, GS30.log, CSUSHPINSA.log, FIPS, starts_with("Time")) %>%
  colnames()
formula.pois<-as.formula(paste("Sales ~ ", paste(Var_list,collapse="+"), " + offset(log(huest))"))
...
Var_list.noeq<-Main_Data.pois %>% 
  ...
  colnames()
formula.pois.noeq<-as.formula(paste("Sales ~ ", paste(Var_list.noeq,collapse="+"), " + offset(log(huest))"))
...
Main_Data.pois$Sales.Predicted<-model.pois$fitted.values
Main_Data.pois$Sales.Rate <- Main_Data.pois$Sales.Predicted/Main_Data.pois$huest
...
Main_Data.pois$Sales.Rate.Change <- (Main_Data.pois$Sales.Predicted - Main_Data.pois$withoutEQ)/Main_Data.pois$huest
...
Main_Data.pois<- Main_Data.pois %>% 
  group_by(FIPS) %>% 
  mutate(Sales.Rate.SD = sd(Sales/huest),
         Sales.Rate.Mean = mean(Sales/huest),
         Sales.Rate.Change.S=(Sales.Predicted - withoutEQ)/huest/Sales.Rate.SD,
         Sales.Rate.Change.M= (Sales.Predicted - withoutEQ)/huest/Sales.Rate.Mean)
...
saveGIF(
  for(Year in unique(Main_Data.pois$YYYYMM)){
    temp<-merge(counties, Main_Data.pois %>% filter(YYYYMM==Year) %>% dplyr::select(FIPS, YYYYMM, Sales.Rate.Change.S), by.x = "GEOID", by.y = "FIPS")
    print(
      spplot(temp, zcol = "Sales.Rate.Change.S", col.regions = c("red", "orange", "white", "white", "blue", "purple"),
             main=list(label=paste("Sales Rate #SD Change from Earthquakes", Year),cex=1.5), at = c(-2,-1,-.25,0,.25,1,2))
    )
  },
  movie.name = "poisSD_Predictions_Sales.gif", interval=.3
)

saveGIF(
  for(Year in unique(Main_Data.pois$YYYYMM)){
    temp<-merge(counties, Main_Data.pois %>% filter(YYYYMM==Year) %>% dplyr::select(FIPS, YYYYMM, Sales.Rate.Change.M), by.x = "GEOID", by.y = "FIPS")
    print(
      spplot(temp, zcol = "Sales.Rate.Change.M", col.regions = c("red", "orange", "white", "white", "blue", "purple"),
             main=list(label=paste("Sales Rate % of FIPS Mean Change from Earthquakes", Year),cex=1), at = c(-.25,-.1,-.05,0,.05,.1,.25))
    )
  },
  movie.name = "poisMU_Predictions_Sales.gif", interval=.3
)


##Example regression: Sales as a percentage of total housing units
...
Main_Data.glmnet<-Main_Data %>%
  mutate(Time2 = Time^2,
         ...
         GS30.log = log(GS30),
         CSUSHPINSA.log = log(CSUSHPINSA))%>%
  filter(complete.cases(Sales, huest, EQ_L21_6))%>%
  select_if(~ !any(is.na(.)))
...
Model.glmnet<-cv.glmnet(y=as.vector(Main_Data.glmnet$Sales),x=Main_Data.glmnet_x, offset = as.vector(log(Main_Data.glmnet$huest)), 
                        family = "poisson", alpha = .9)

##Extract predicted new Sales on the input data, using the lambda that minimized the objective function of cv.glmnet 
Main_Data.glmnet$Sales.Predicted<-predict(object=Model.glmnet, newx = Main_Data.glmnet_x, 
...
Main_Data.glmnet$Sales.Rate <- Main_Data.glmnet$Sales.Predicted/Main_Data.glmnet$huest
...
Main_Data.glmnet$Sales.Rate.Change <- (withEQ - withoutEQ)/Main_Data.glmnet$huest
#normalize change in the sales rate to the response standard deviation within the county over the modeled time periods. 
...
saveGIF(
  for(Year in unique(Main_Data.glmnet$YYYYMM)){
    temp<-merge(counties, Main_Data.glmnet %>% filter(YYYYMM==Year) %>% dplyr::select(FIPS, YYYYMM, Sales.Rate.Change.S), by.x = "GEOID", by.y = "FIPS")
    print(
      spplot(temp, zcol = "Sales.Rate.Change.S", col.regions = c("red", "orange", "white", "white", "blue", "purple"),
             main=list(label=paste("Sales Rate #SD Change from Earthquakes", Year),cex=1.5), at = c(-2,-1,-.5,0,.5,1,2))
    )
  },
  movie.name = "GlmnetSD_Predictions_Sales.gif", interval=.3
)

saveGIF(
  for(Year in unique(Main_Data.glmnet$YYYYMM)){
    temp<-merge(counties, Main_Data.glmnet %>% filter(YYYYMM==Year) %>% dplyr::select(FIPS, YYYYMM, Sales.Rate.Change.M), by.x = "GEOID", by.y = "FIPS")
    print(
      spplot(temp, zcol = "Sales.Rate.Change.M", col.regions = c("red", "orange", "white", "white", "blue", "purple"),
             main=list(label=paste("Sales Rate % of FIPS Mean Change from Earthquakes", Year),cex=1), at = c(-.25,-.1,-.05,0,.05,.1,.25))
    )
  },
  movie.name = "GlmnetMU_Predictions_Sales.gif", interval=.3
)

##Point of interest: Can we detect spatial autocorrelation? 
##Counterfactual above tends to show evidence that Sales increase in regions adjacent to declines. 
...
##Calculate Moran's I, a measure of spatial autocorrelation, based on the residual Sales rates
...
ape::Moran.I(as.vector((Main_Data.glmnet$Sales.Predicted-Main_Data.glmnet$Sales)/Main_Data.glmnet$huest), weight=weight, scaled = TRUE, na.rm = FALSE,
...
formula<-as.formula(paste("Sales ~", paste(useful_vars, collapse = "+"),"+",additional_vars, sep = " "))
...
formula.gam<-as.formula(paste("Sales ~ ", paste(Var_list,collapse="+"), "+ s(Time,df=6) + FIPS + offset(log(huest))"))
...
#estimate with EQ - estimate without EQ = net change in sales rate from seismicity
##Yes, there's still some natural seismicity in the region, so this isn't a perfect exercise. It should be adequate, though. 
Main_Data.gam$Sales.Predicted<-model.gam$fitted.values
Main_Data.gam$Sales.Rate <- Main_Data.gam$Sales.Predicted/Main_Data.gam$huest
...
Main_Data.gam$Sales.Rate.Change <- (Main_Data.gam$Sales.Predicted - Main_Data.gam$withoutEQ)/Main_Data.gam$huest
...
saveGIF(
  for(Year in unique(Main_Data.gam$YYYYMM)){
    temp<-merge(counties, Main_Data.gam %>% filter(YYYYMM==Year) %>% dplyr::select(FIPS, YYYYMM, Sales.Rate.Change.S), by.x = "GEOID", by.y = "FIPS")
    print(
      spplot(temp, zcol = "Sales.Rate.Change.S", col.regions = c("red", "orange", "white", "white", "blue", "purple"),
             main=list(label=paste("Sales Rate #SD Change from Earthquakes", Year),cex=1.5), at = c(-2,-1,-.25,0,.25,1,2))
    )
  },
  movie.name = "GamSD_Predictions_Sales.gif", interval=.3
)

saveGIF(
  for(Year in unique(Main_Data.gam$YYYYMM)){
    temp<-merge(counties, Main_Data.gam %>% filter(YYYYMM==Year) %>% dplyr::select(FIPS, YYYYMM, Sales.Rate.Change.M), by.x = "GEOID", by.y = "FIPS")
    print(
      spplot(temp, zcol = "Sales.Rate.Change.M", col.regions = c("red", "orange", "white", "white", "blue", "purple"),
             main=list(label=paste("Sales Rate % of FIPS Mean Change from Earthquakes", Year),cex=1), at = c(-.25,-.1,-.05,0,.05,.1,.25))
    )
  },
  movie.name = "GamMU_Predictions_Sales.gif", interval=.3
)
